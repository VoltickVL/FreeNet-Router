...
UPDATER_MARK="$TMP/updater.called"
...
# Healthy application route is distinct from endpoint TCP and persists no secret material.
QUALITY_MODE=healthy; export QUALITY_MODE
run_probe > "$TMP/healthy.out"
grep -Fq 'VPN_PROCESS=UP' "$TMP/healthy.out"
grep -Fq 'ENDPOINT_REACHABLE=yes' "$TMP/healthy.out"
grep -Fq 'VPN_ROUTE_OK=yes' "$TMP/healthy.out"
grep -Fq 'VPN_QUALITY=HEALTHY' "$TMP/healthy.out"
grep -Fq 'RTT_MS=85' "$TMP/healthy.out"
grep -Fq 'FAILURE_STREAK=0' "$TMP/healthy.out"
grep -Fq 'MUTATION=NONE' "$TMP/healthy.out"
[ "$(stat -c '%a' "$STATE_FILE")" = 600 ]
! grep -Eq 'vless://|subscription|uuid|publicKey|shortId' "$STATE_FILE"

# High RTT is degraded even when the application route still succeeds.
...
# A TCP-live endpoint with failed application route is not HEALTHY.
...
# Repeated route failures become DOWN without any endpoint mutation.
...
# Unsafe/unconstructable probe path is UNKNOWN and does not invent a route failure.
