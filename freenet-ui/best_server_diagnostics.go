...
func bestServerTransferIssue(output string, err error) string {
	...
	if len(fields) >= 2 {
		if n, e := strconv.Atoi(fields[0]); e == nil && n >= 100 && n <= 599 {
			code = n
		}
		if n, e := strconv.ParseFloat(fields[1], 64); e == nil && n >= 0 && n <= 1024*1024*1024 {
			bytes = int64(n)
		}
	}
	exitCode := -1
...
func bestServerRejectionReasons(c bestServerQualityCandidate) []string {
	if !c.Tested {
		return []string{"Полная проверка качества не выполнялась"}
	}
	if !c.Available {
