;(() => {
  ...
  async function action(name) {
    try {
      ...
      if (action === 'backup_restore' && !confirm('Восстановить последний внутренний снимок настроек FreeNet? Перед восстановлением будет создан аварийный снимок текущих файлов.')) return;
  ...
  function renderJournalSummary(events) {
    ...
    host.innerHTML = `
      ...
      <div class="fn3-journal-stat"><span>Без изменений / пропущено</span><strong>${neutral}</strong></div>
  ...
  function mountJournalPage() {
    ...
    page.innerHTML = `<h1>Журнал</h1><p>История AUTO VPN, обновлений, GeoData и резервного копирования FreeNet. Здесь показаны последние 30 событий без секретов и credential-bearing данных.</p><div id="fn3JournalSummary" class="fn3-journal-summary"></div><section class="fn3-card"><table class="fn3-table"><thead><tr><th>Время</th><th>Событие</th><th>Результат</th><th>Комментарий</th></tr></thead><tbody id="fn3JournalFull"></tbody></table></section>`;
