...
(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      #fnVpnPickerPopover .fn-vpn-picker-body,#fnVpnPickerPopover #bestServerAdvanced,#fnVpnPickerPopover #profilesList{scrollbar-gutter:stable}
  ...
  function syncChip(status) {
    ...
    if (!code) {
      const match = String(status.profile_label || '').trim().match(/^([A-Za-z]{2})\\s+/);
