(() => {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      .auth-wrap{
        position:fixed!important;inset:0!important;max-width:none!important;margin:0!important;
        display:grid!important;place-items:center!important;padding:24px!important;
        background:radial-gradient(circle at 68% -15%,#17345c 0,#0d1d32 42%,#081523 72%,#07101b 100%)!important;
      ...
      .fn-update-popover[hidden]{display:none!important}
      .fn-update-arrow{position:absolute;top:-7px;right:29px;width:13px;height:13px;transform:rotate(45deg);background:#102746;border-left:1px solid #31577d;border-top:1px solid #31577d}
      .fn-update-head{display:flex;align-items:center;justify-content:space-between;gap:12px;position:relative}
      .fn-update-title{display:flex;align-items:center;gap:9px;font-size:16px;font-weight:800}
      .fn-update-badge{display:grid;place-items:center;width:27px;height:27px;border-radius:50%;background:#31e5a2;color:#052319;font-size:18px;font-weight:900}
      .fn-update-close{appearance:none;border:0;background:transparent;color:#aac0dc;font-size:23px;line-height:1;cursor:pointer;padding:2px 4px}
      .fn-update-versions{display:grid;grid-template-columns:auto 1fr;gap:7px 13px;margin-top:16px;font-size:13px}.fn-update-versions span{color:#b3c2d6}.fn-update-versions strong{color:#f7f9fd}.fn-update-versions strong.available{color:#36e3a2}
      .fn-update-copy{margin:11px 0 0;color:#c6d2e2;font-size:12.5px;line-height:1.5}
      .fn-update-safe{display:flex;align-items:flex-start;gap:9px;margin-top:12px;padding:10px 11px;border:1px solid rgba(54,227,162,.32);border-radius:11px;background:rgba(20,112,81,.18);color:#d7faec;font-size:11.5px;line-height:1.4}.fn-update-safe b{display:block;color:#42e7aa}.fn-update-safe-icon{font-size:18px;line-height:1}
      .fn-update-status{margin-top:11px;padding:9px 10px;border:1px solid #294969;border-radius:10px;background:#09192b;color:#b9c9dc;font-size:11.5px;line-height:1.45;white-space:pre-line}.fn-update-status.ok{border-color:rgba(54,227,162,.32);color:#c9f6e4}.fn-update-status.bad{border-color:rgba(255,92,106,.38);color:#ffd1d6}
      .fn-update-progress{width:100%;margin-top:10px;accent-color:#36e3a2}
  ...
  function ensurePopover() {
    ...
    root.innerHTML = `
      ...
      <div class="fn-update-head">
        <div class="fn-update-title"><span class="fn-update-badge">↑</span><span>Обновление FreeNet</span></div>
        <button id="fnUpdateClose" class="fn-update-close" type="button" aria-label="Закрыть">×</button>
      </div>
      <div class="fn-update-versions">
        <span>Текущая версия:</span><strong id="fnUpdateCurrent">—</strong>
        <span>Доступна новая версия:</span><strong id="fnUpdateLatest" class="available">проверяем…</strong>
      </div>
      <p id="fnUpdateCopy" class="fn-update-copy">Проверка обновления выполняется прямо здесь, без перехода в отдельный раздел.</p>
      ...
      <div class="fn-update-actions">
        <button id="fnUpdateCheck" class="btn secondary" type="button">Проверить</button>
        <button id="fnUpdateApply" class="btn fn-update-apply" type="button" disabled>Обновить</button>
      </div>`;
  ...
  function renderPlan(plan) {
    updatePlan = plan;
    const current = qs('#fnUpdateCurrent');
    const latest = qs('#fnUpdateLatest');
    if (current) current.textContent = plan.current_version || '—';
    if (latest) {
      latest.textContent = plan.update_available ? (plan.latest_version || plan.target_tag || 'доступна') : 'не требуется';
      latest.className = plan.update_available ? 'available' : '';
    }
    const apply = qs('#fnUpdateApply');
    if (apply) {
      apply.disabled = !(plan.success && plan.ready && plan.update_available && plan.target_tag);
      apply.textContent = plan.update_available && plan.target_tag ? `Обновить` : 'Обновить';
    }
    const control = qs('#topFreenetUpdate');
    if (control) control.classList.toggle('update-available', !!plan.update_available);
  }
  ...
  async function checkUpdate() {
    ...
    try {
      ...
      if (!response.ok || !plan.success) throw new Error(plan.error || 'Не удалось проверить обновление');
      renderPlan(plan);
      if (plan.update_available) {
        setStatus(`Доступно ${plan.latest_version || plan.target_tag}. SHA-256: ${plan.manifest_verified ? 'проверен' : 'не подтверждён'}.`, plan.manifest_verified ? 'ok' : '');
      } else {
        setStatus('Установлена актуальная версия FreeNet.', 'ok');
      }
    } catch (error) {
      setStatus(error && error.message ? error.message : 'Не удалось проверить обновление.', 'bad');
    } finally {
      setProgress(false);
      setBusy(false);
    }
  ...
  async function waitForVersion(target) {
    ...
    setStatus(`Целевая версия ${target} не подтверждена браузером. Не запускайте обновление повторно до проверки фактического состояния.`, 'bad');
    return false;
  }
    ...
    while (updatePolling && Date.now() < deadline) {
      try {
        ...
        if (response.ok) {
          ...
          setStatus(lines.join('\n'), state.state === 'SUCCESS' ? 'ok' : (state.state === 'FAILED' || state.state === 'ROLLBACK_FAILED') ? 'bad' : '');
  ...
  async function startUpdate() {
    ...
    try {
      const response = await fetch('/api/system/update/apply', {
        ...
        body: JSON.stringify({target_tag: updatePlan.target_tag})
      });
      ...
      setProgress(false);
      setBusy(false);
      setStatus(error && error.message ? error.message : 'Обновление не запущено.', 'bad');
    }
  }
  ...
  function openPopover() {
    ...
    const current = qs('#fnUpdateCurrent');
    if (current && current.textContent === '—') current.textContent = String(versionText).replace(/^FreeNet UI\s*/i, '');
    checkUpdate();
  }

  function interceptLegacyUpdateModal() {
    ...
    document.addEventListener('click', event => {
      const control = event.target && event.target.closest ? event.target.closest('#topFreenetUpdate') : null;
      if (!control) return;
  ...
  function mount() {
    ...
    mountRememberMe();
    ensurePopover();
    interceptLegacyUpdateModal();
    const observer = new MutationObserver(() => mountRememberMe());
