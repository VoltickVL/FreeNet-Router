(() => {
  ...
  function patchStatusRendering() {
    ...
    updateStatusViews = function(s) {
      ...
      if (dnsState) dnsState.textContent = xrayDNS ? (s.dns_out_present ? 'Раздельный' : 'Раздельный · требует внимания') : 'Прямой';
  ...
  async function mountSplitDNSMemoryGate(){const select=qs('#dnsModeSelect');if(!select)return;const option=select.querySelector('option[value="xkeen"]');if(!option)return;option.disabled=true;option.textContent='Раздельный — проверка ОЗУ…';try{const r=await fetch('/api/capabilities',{cache:'no-store'});if(r.status===401){if(typeof loadAuthStatus==='function')await loadAuthStatus();splitDNSCapabilityAttempts++;if(splitDNSCapabilityAttempts<20)setTimeout(mountSplitDNSMemoryGate,1500);return;}const capability=await r.json();const supported=!!(r.ok&&capability&&capability.success&&capability.split_dns_supported);const oldNotice=qs('#splitDNSMemoryNotice');if(supported){option.disabled=false;option.textContent='Раздельный';if(oldNotice)oldNotice.remove();return;}option.disabled=true;option.textContent='Раздельный — недоступно (мало ОЗУ)';const mem=Number(capability&&capability.memory_total_mib)||0;const min=Number(capability&&capability.split_dns_min_mib)||768;const reason=String((capability&&capability.reason)||'').trim();showSplitDNSMemoryNotice(reason||(mem?`Режим «Раздельный» недоступен: обнаружено ${mem} MiB RAM, требуется не менее ${min} MiB. Используйте режим «Прямой».`:'Режим «Раздельный» недоступен: объём RAM не удалось безопасно определить. Используйте режим «Прямой».'));}catch(_){option.disabled=true;option.textContent='Раздельный — недоступно';showSplitDNSMemoryNotice('FreeNet не смог подтвердить достаточный объём RAM. Режим «Раздельный» заблокирован; используйте «Прямой».');}}
